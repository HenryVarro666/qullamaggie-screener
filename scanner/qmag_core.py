...
def pick_streaks(date_str, tickers):
    """连续在榜天数：每只 ticker 从今天往前数，连续出现在历史精选的报告日数（含今天）。更新历史并返回 {ticker: streak}。
    历史存 OUTDIR/picks_history.json（{date:[tickers]}，留近 120 个报告日）。REPORT_DATE 重生成旧日也正确（只数其之前的日）。"""
    p=os.path.join(OUTDIR,"picks_history.json")
    try: hist=rd_json(p)
    except Exception: hist={}
    prior=sorted([d for d in hist if d<date_str], reverse=True)     # 严格早于当天的报告日，由近及远
    out={}
    for t in tickers:
        s=1
        for d in prior:
            if t in hist.get(d,[]): s+=1
            else: break
        out[t]=s
    hist[date_str]=list(tickers)
    for d in sorted(hist)[:-120]: hist.pop(d,None)
    try:
        os.makedirs(OUTDIR, exist_ok=True)
        with open(p,"w",encoding="utf-8") as f: json.dump(hist,f,ensure_ascii=False)
